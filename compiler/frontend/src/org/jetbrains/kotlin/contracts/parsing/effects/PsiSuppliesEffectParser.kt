...
internal class PsiSuppliesEffectParser(
    ...
    override fun tryParseEffect(expression: KtExpression): EffectDeclaration? {
        ...
        val supplier = ContextualEffectSystem.ALL_PARSERS.asSequence()
            .map { it(trace.bindingContext) }
            .map { parser -> parser.parseDeclarationForSupplier(expression) }
            .filterNotNull()
            .firstOrNull() ?: return null
        return SuppliesContextualEffectDeclaration(supplier)
    }
}